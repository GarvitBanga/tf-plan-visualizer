...
exports.getTopChanges = getTopChanges;
function parsePlan(planJson) {
    ...
    for (const change of planJson.resource_changes) {
        ...
        else if (change.change.actions.includes('create') && change.change.actions.includes('delete')) {
            risks.push({
                type: 'replace',
                resource: change.address,
                reason: change.action_reason
...
function parseConfiguration(config, resources, edges, modules) {
    // Parse root module
...
function parseModule(module, modulePath, resources, edges, modules) {
...
function extractReferencesFromExpressions(expressions, resourceAddress, edges) {
    if (!expressions)
        return;
...
function findReferencesInExpression(expr) {
    const references = [];
...
function getResourcesByModule(resources) {
    const byModule = {};
    for (const resource of resources) {
...
function getTopChanges(resources, limit = 10) {
    return resources
...
function getActionPriority(actions) {
